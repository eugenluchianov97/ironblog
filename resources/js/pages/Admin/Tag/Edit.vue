...
import {me, tagIndex, tagShow, tagUpdate} from "@/api/api.js";
export default {
    name:"AdminTagEdit",
    components:{
        AdminLayout
    ...
    mounted(){
        tagShow(this.id).then(res => {
            if(res.status === 200){
                this.form.name.value = res.data.tag.name
                this.form.name_ro.value = res.data.tag.name_ro
                this.form.name_en.value = res.data.tag.name_en
            }
        })
    },
    methods:{
        submit(){
            ...
            tagUpdate(this.id,data).then(res => {
                if(res.status === 200){
                    this.$notify({
                        type: "success",
                        text: 'Tag updated successfuly!!',
                        title: "Authorization",
                    });
